//====================== Copyright 2017-2023, HTC.Corporation. All rights reserved. ======================
/**
...
namespace HTC.UnityPlugin.Vive3DSP
{
    ...
    public static class Vive3DSPAudio
    {
        ...
        private static List<Vive3DSPAudioRaycastOcclusion> raycastOccList = new List<Vive3DSPAudioRaycastOcclusion>();
        private static List<Vive3DSPAudioGeometricOcclusion> geoOccList = new List<Vive3DSPAudioGeometricOcclusion>();
        ...
        public static void SetReset(bool isReset)
        {
            Vive3DSPAudioNative.Set3DSPReset(isReset);
        }

        public static void UpdateOcclusionCoverRatio(Vive3DSPAudioListener listener)
        {
            // filter out the preserved raycast occlusion
            var preservedRaycast = raycastOccList.Where(occ => occ.OcclusionGeometry == OccGeometryMode.Preserved && occ.OcclusionEffect == true);
            // filter out non virtual audio source
            var sources = srcList.Where(src => src.isVirtual == false && src.Occlusion == true);

            foreach (var raycast in preservedRaycast)
            {
                foreach (var src in sources)
                {
                    var ratio = 0.0f;
                    if(raycast.OcclusionCompEffect == true){
                        var srcToLeftDir = listener.transform.position - src.transform.position;
                        ratio = getRaycastOcclusionRatio(src.transform.position, srcToLeftDir, raycast);
                    }
                    src.AddRaycastRatio(raycast.Id, ratio);
                }
            }
        }
        
        public static int CreateRaycastOcclusion(Vive3DSPAudioRaycastOcclusion occ)
        {
            var id = Vive3DSPAudioNative.Create3DSPOcclusionObject();
            if (id != -1)
            {
                raycastOccList.Add(occ);
            }
            return id;
        }

        public static void DestroyRaycastOcclusion(Vive3DSPAudioRaycastOcclusion occ)
        {
            if (occ.Id != -1)
            {
                Vive3DSPAudioNative.Destroy3DSPOcclusionObject(occ.Id);
            }
            raycastOccList.Remove(occ);
        }

        public static int CreateGeometricOcclusion(Vive3DSPAudioGeometricOcclusion occ)
        {
            var id = Vive3DSPAudioNative.Create3DSPOcclusionObject();
            if (id != -1)
            {
        ...
        public static void DestroyGeometricOcclusion(Vive3DSPAudioGeometricOcclusion occ)
        {
            if (occ.Id != -1)
            {
                Vive3DSPAudioNative.Destroy3DSPOcclusionObject(occ.Id);
            }
            geoOccList.Remove(occ);
        }

        private static float getRaycastOcclusionRatio(Vector3 srcPos, Vector3 direction, Vive3DSPAudioRaycastOcclusion occ)
        {
            float maxRayBiasAngle = occ.OcclusionAngle;
            float minRayBiasAngle = occ.OcclusionAngle, totalRays = occ.RayNumber;
            int rayEmitLayer = 1, rayDisperseOrder = occ.RayNumber - 1;
            if (occ.RayNumber >= 15)
            {
                minRayBiasAngle = occ.OcclusionAngle / 2.0f;
                rayEmitLayer = 2;
            ...
            int sourceColliderNumber = Physics.RaycastAll(srcPos, direction, dist).Count(h => h.transform == occ.transform);
            int listenerColliderNumber = sourceColliderNumber;
            if (occ.RayNumber == 1)
            {
                return sourceColliderNumber == 0 ? 0.0f : 1.0f;
            }

            var maxDirectForwardUp = Quaternion.AngleAxis(maxRayBiasAngle, Vector3.up) * direction;
            var maxDirectUp = maxDirectForwardUp - direction;
            var minDirectForwardUp = Quaternion.AngleAxis(minRayBiasAngle, Vector3.up) * direction;
            var minDirectUp = minDirectForwardUp - direction;
            var rayDisperseAngle = 360f / rayDisperseOrder;
            var maxminDirectUpMagRatio = maxDirectUp.magnitude / minDirectUp.magnitude;
            for (int i = 0; i < rayEmitLayer; i++)
            {
                Vector3 directUp;
                if (i == 0)
                {
                    directUp = minDirectUp;
                }
                else if (i == rayEmitLayer - 1)
                {
                    directUp = maxDirectUp;
                }
                else
                {
                    directUp = (i * (maxminDirectUpMagRatio - 1) / rayEmitLayer + 1) * minDirectUp;
                }
                
                for (int j = 0; j < rayDisperseOrder; j++)
                {
                    var rotUp = Quaternion.AngleAxis(rayDisperseAngle * j, direction) * directUp;
                    {
                        // source part
                        var forwardUp = direction + rotUp;
                        listenerColliderNumber += Physics.RaycastAll(srcPos, forwardUp, forwardUp.magnitude).Count(h => h.transform == occ.transform);
                    }
                    {
                        // listener part
                        var oriAxis = srcPos + direction;
                        var forwardUp = -direction + rotUp;
                        sourceColliderNumber += Physics.RaycastAll(oriAxis, forwardUp, forwardUp.magnitude).Count(h => h.transform == occ.transform);
                    }
                }
            }

            return (float)Math.Max(sourceColliderNumber, listenerColliderNumber) / totalRays;
        }
        ...
        public static void PassObjToNative()
        {
            ...
            foreach (var geo in geoOccList)
            {
                Vive3DSPAudioNative.Set3DSPOcclusionProperty(geo.Id, geo.PropPtr);
            }

            foreach (var ray in raycastOccList)
            {
                Vive3DSPAudioNative.Set3DSPOcclusionProperty(ray.Id, ray.PropPtr);
            }
        }
