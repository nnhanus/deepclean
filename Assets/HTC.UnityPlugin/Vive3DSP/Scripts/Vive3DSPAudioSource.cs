...
namespace HTC.UnityPlugin.Vive3DSP
{
    ...
    public class Vive3DSPAudioSource : MonoBehaviour
    {
        ...
        private List<int> raycastId = new List<int>();
        private List<float> raycastCoverRatio = new List<float>();
        ...
        /// <summary>
        /// Pass raycast cover ratio infomation to SourceProp.
        /// It would be called when 3DSPAudio.LateUpdate has be forked
        /// 
        /// Steps:
        /// 1. Free the last used un-management space
        /// 2. Set raycast count to SourceProp.raycastCount
        /// 3. Allocate un-management space and assign it to SourceProp.raycastCoverRatio(R)
        /// 4. Copy raycast raycastCoverRatio to un-management space
        /// </summary>
        private void transferRaycastCoverRatio()
        {
            sourceProp.raycastCount = raycastId.Count;

            Marshal.FreeHGlobal(sourceProp.occluderId);
            sourceProp.occluderId = IntPtr.Zero;
            if (raycastId.Count != 0)
            {
                sourceProp.occluderId = Marshal.AllocHGlobal(raycastId.Count * sizeof(int));
                Marshal.Copy(raycastId.ToArray(), 0, sourceProp.occluderId, raycastId.Count);
                raycastId.Clear();
            }

            Marshal.FreeHGlobal(sourceProp.raycastCoverRatio);
            sourceProp.raycastCoverRatio = IntPtr.Zero;
            if (raycastCoverRatio.Count != 0)
            {
                sourceProp.raycastCoverRatio = Marshal.AllocHGlobal(raycastCoverRatio.Count * sizeof(float));
                Marshal.Copy(raycastCoverRatio.ToArray(), 0, sourceProp.raycastCoverRatio, raycastCoverRatio.Count);
                raycastCoverRatio.Clear();
            }
        }

        public void AddRaycastRatio(int occId, float ratio)
        {
            raycastId.Add(occId);
            raycastCoverRatio.Add(ratio);
        }
